...
class OrderSummaryController extends GetxController {
  ...
  Future<void> sendOrderSummaryToWhatsApp(BuildContext context) async {
    ...
    await whatsappProvider.sendWhatsAppMessage(message, businessWhatsAppNumber);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('Su pedido ha sido enviado al restaurante.'),
          actions: <Widget>[
            TextButton(
              child: const Text('Aceptar'),
              onPressed: () {
                // Limpiar el resumen de la orden
                orderSummary.value.orderItems.clear();
                orderSummary.value.totalAmount = 0.0;
                totalAmount = 0.0.obs;
                totalProducts = 0.obs;
                orderSummary.refresh();
                ...
                // Navegar a SliverHomeScreen
                Navigator.of(context).pushAndRemoveUntil(
                  MaterialPageRoute(builder: (context) => SliverHomeScreen()),
